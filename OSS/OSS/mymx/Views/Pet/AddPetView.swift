...
struct AddPetView: View {
    ...
    var body: some View {
        ...
        Form{
            Section(content: {
                HStack{
                    Text("名称")
                    Spacer()
                    TextField("请输入身份名称", text: $petInfo.name)
                ...
                TextField("描述一下自己吧...", text: $petInfo.description)
                    .foregroundStyle(.secondary)
                    .frame(minHeight: 80)
                    .submitLabel(.done)
            }, header: {Text("身份信息（必填）")}, footer: ({
                HStack{
                    Spacer()
                    if(addPetVM.loading){
                        ProgressView("身份添加中...")
        ...
        .toolbar{
            NavigationLink("编辑身份", destination: {
                PetListView()
                .navigationTitle("身份列表")
            })
        }
